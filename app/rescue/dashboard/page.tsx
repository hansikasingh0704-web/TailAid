...
import { CheckCircle, Clock, AlertCircle, MapPin, Phone, User } from 'lucide-react'
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Only show alerts that haven't been accepted by hospitals yet
          const rescueAlerts = allAlerts.filter((alert: any) => 
            !alert.acceptedByRole || alert.acceptedByRole === 'Rescue Center'
          )
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#0D0D0D] via-[#1A1A1A] to-[#0D0D0D]">
      ...
      <div className="backdrop-blur-md bg-white/5 border-b border-white/10 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <h1 className="text-2xl font-bold text-white">Rescue Center Dashboard</h1>
          <p className="text-gray-400 text-sm">Respond to emergency alerts</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 backdrop-blur-md bg-white/5 border border-white/20 rounded-2xl p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white">{alerts.length} Pending Requests</h3>
              <p className="text-gray-400 text-sm">Accept requests to help animals in your area</p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="backdrop-blur-md bg-white/5 border border-white/20 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-400">No pending emergency requests at the moment</p>
          </div>
        ) : (
